...
var qs = {};
    ...
    qs.name = window.getQueryString('name') || "Phone" + (window.getQueryString('id') ? (" " + window.getQueryString('id')) : "");	// name of app in spacebrew
    qs.server = window.getQueryString('server') || 'hyperspacebrew.com';    // name of spacebrew server
...
function setup (){
  ...
  var name = window.prompt("Name?", qs.name + ' ' + random_id.substring(random_id.length-4));

  $("header > h1").text(name);

  // setup spacebrew
  if (debug) console.log("[setup] setting up connection to Spacebrew");
  sb.connection = new Spacebrew.Client(qs.server, name, qs.description);
...
var registerUserEventListeners  = function () {
  ...
  // Button
  sb.connection.addPublish( "buttonPress", "boolean", "false" );

  $("#buttonMsg").on("mousedown", onButtonPress);
  $("#buttonMsg").on("mouseup", onButtonRelease);

  // Slider
  sb.connection.addPublish("slider", "range", "0");

  $(".slider").bind("change", function(event, ui) {
    if (values[event.target.id] != event.target.value) {
      var value = mapVal(event.target.value, 0, 1023, -1, 1);
      console.log("[change] value " + value);
      sb.connection.send(event.target.id, "range", value);
...
var onButtonPress = function (evt) {
      ...
      alert("[onButtonPress] button has been pressed");
      sb.connection.send("buttonPress", "boolean", "true");
}
...
/**
   ...
   * @param  {Event object} evt Holds information about the button press event
   */
var onButtonRelease = function (evt) {
      console.log("[onButtonRelease] button has been released"); 
      sb.connection.send("buttonPress", "boolean", "false");
}
